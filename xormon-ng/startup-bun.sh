...
# EXPERIMENT: upstream's "npm run start:runtime" is
#   node dist/verify.js && pm2-runtime production.config.js
# which cannot be used here - there is no npm, and the script body calls node by
# name. Run the same two steps directly through bun instead.
exec runuser -u xormon -- bash -lc "cd '$APPDIR/server-nest' \
    && bun dist/verify.js \
    && exec bun node_modules/pm2/bin/pm2-runtime production.config.js"
